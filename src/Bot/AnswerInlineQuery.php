...
trait AnswerInlineQuery {
    ...
    public function answerInlineQuery(string|int $inline_id, array|string $results = [], array $conf = []): static {
        ...
        $conf['results'] = $results;// is_array($results) ? json_encode($results, JSON_UNESCAPED_UNICODE + JSON_UNESCAPED_SLASHES) : $results;
        $data = $this->get_conf([
            // 唯一标识已回答的查询
            'inline_query_id',
    ...
    public function answerInlinePhotoConfig(mixed $photoUrl, string $text = '', mixed $thumb = "", array $conf = []): array {
        ...
        if (!empty($thumb = ($conf['thumbnail_url'] ?? $thumb))) {
            $conf["thumbnail_url"] = $thumb;
        }
